...
vector<pair<ll, ll>> possne(int j, int k) {
...
int main() {
	...
	fora(j, 60) fora(l, 60) fora(k, 1 << 15) {
		dp[0][j][l][k] = inf;
	}
	...
	fora(ii, 26) {
		...
		ll qq = dp[i][j][l][k] + np(k);
		vector<pair<ll, ll>> me = possne(j, k);
		vector<pair<ll, ll>> el = possne(l, k);
		forc(u, me) forc(v, el) {
			imax(dp[(i + 1) % 2][u.first][v.first][u.second | v.second], qq);
		}
	}}
